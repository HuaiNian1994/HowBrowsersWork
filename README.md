...
3. 解析和 DOM 树构建
   1. 解析 - 综述
      1. [语法](#语法)
      2. [解析器和词法分析器的组合](#解析器和词法分析器的组合)
      3. [翻译](#翻译)
      4. [解析示例](#解析示例)
      5. [词汇和语法的正式定义](#词汇和语法的正式定义)
      6. [解析器类型](#解析器类型)
      7. [自动生成解析器](#自动生成解析器)
   2. HTML 解析器
      1. [HTML 语法定义](#HTML 语法定义)
      2. [非与上下文无关的语法](#非与上下文无关的语法)
      3. [HTML DTD](#HTML_DTD)
      4. [DOM](#DOM)
      5. [解析算法](#解析算法)
      6. [标记化算法](#标记化算法)
      ...
      9. [浏览器的容错机制](#浏览器的容错机制)
   3. CSS 解析
      1. [WebKit CSS 解析器](#WebKit CSS 解析器)
   4. 处理脚本和样式表的顺序
      1. [脚本](#脚本)
      2. [预解析](#预解析)
      3. [样式表](#样式表)
4. 呈现树构建
   1. [呈现树和 DOM 树的关系](#呈现树和 DOM 树的关系)
   2. [构建呈现树的流程](#构建呈现树的流程)
   3. 样式计算
      1. [共享样式数据](#共享样式数据)
      2. [Firefox 规则树](#Firefox 规则树)
         1. [结构划分](#结构划分)
         2. [使用规则树计算样式上下文](#使用规则树计算样式上下文)
      3. [对规则进行处理以简化匹配](#对规则进行处理以简化匹配)
      4. [以正确的层叠顺序应用规则](#以正确的层叠顺序应用规则)
         1. [样式表层叠顺序](#样式表层叠顺序)
         2. [特异性](#特异性)
         3. [规则排序](#规则排序)
   4. [渐进式处理](#渐进式处理)
5. 布局
   1. [Dirty 位系统](#Dirty 位系统)
   2. [全局布局和增量布局](#全局布局和增量布局)
   3. [异步布局和同步布局](#异步布局和同步布局)
   4. [优化](#优化)
   5. [布局处理](#布局处理)
   6. [宽度计算](#宽度计算)
   7. [换行](#换行)
6. [绘制](#绘制)
   1. [全局绘制和增量绘制](#全局绘制和增量绘制)
   2. [绘制顺序](#绘制顺序)
   3. [Firefox 显示列表](#Firefox 显示列表)
   4. [WebKit 矩形存储](#WebKit 矩形存储)
7. [动态变化](#动态变化)
8. [呈现引擎的线程](#呈现引擎的线程)
   1. [事件循环](#事件循环)
9. [CSS2 可视化模型](#CSS2 可视化模型)
   1. [画布](#画布)
   2. [CSS 框模型](#CSS 框模型)
   3. [定位方案](#定位方案)
   4. [框类型](#框类型)
