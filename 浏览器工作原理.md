...
原页面：https://www.html5rocks.com/zh/tutorials/internals/howbrowserswork/

作者：  [Tali Garsiel](https://www.html5rocks.com/profiles/#taligarsiel) and [Paul Irish](https://www.html5rocks.com/profiles/#paulirish) 
...
1. 简介
   1. [我们要讨论的浏览器](#我们要讨论的浏览器)
   2. [浏览器的主要功能](#浏览器的主要功能)
   3. [浏览器的高层结构](#浏览器的高层结构)
2. 呈现引擎
   1. [呈现引擎](#呈现引擎)
   2. [主流程](#The_main_flow)
   3. [主流程示例](#Main_flow_examples)
3. 解析和 DOM 树构建
   1. 解析 - 综述
      1. [语法](#Grammars)
      2. [解析器和词法分析器的组合](#Parser_Lexer_combination)
      3. [翻译](#Translation)
      4. [解析示例](#Parsing_example)
      5. [词汇和语法的正式定义](#Formal_definitions_for_vocabulary_and_syntax)
      6. [解析器类型](#Types_of_parsers)
      7. [自动生成解析器](#Generating_parsers_automatically)
   2. HTML 解析器
      1. [HTML 语法定义](#The_HTML_grammar_definition)
      2. [非与上下文无关的语法](#Not_a_context_free_grammar)
      3. [HTML DTD](#HTML_DTD)
      4. [DOM](#DOM)
      5. [解析算法](#The_parsing_algorithm)
      6. [标记化算法](#The_tokenization_algorithm)
      7. [树构建算法](#Tree_construction_algorithm)
      8. [解析结束后的操作](#Actions_when_the_parsing_is_finished)
      9. [浏览器的容错机制](#Browsers_error_tolerance)
   3. CSS 解析
      1. [WebKit CSS 解析器](#Webkit_CSS_parser)
   4. 处理脚本和样式表的顺序
      1. [脚本](#Scripts)
      2. [预解析](#Speculative_parsing)
      3. [样式表](#Style_sheets)
4. 呈现树构建
   1. [呈现树和 DOM 树的关系](#The_render_tree_relation_to_the_DOM_tree)
   2. [构建呈现树的流程](#The_flow_of_constructing_the_tree)
   3. 样式计算
      1. [共享样式数据](#Sharing_style_data)
      2. Firefox 规则树
         1. [结构划分](#Division_into_structs)
         2. [使用规则树计算样式上下文](#Computing_the_style_contexts_using_the_rule_tree)
      3. [对规则进行处理以简化匹配](#Manipulating_the_rules_for_an_easy_match)
      4. 以正确的层叠顺序应用规则
         1. [样式表层叠顺序](#Style_sheet_cascade_order)
         2. [特异性](#Specificity)
         3. [规则排序](#Sorting_the_rules)
   4. [渐进式处理](#Gradual_process)
5. 布局
   1. [Dirty 位系统](#Dirty_bit_system)
   2. [全局布局和增量布局](#Global_and_incremental_layout)
   3. [异步布局和同步布局](#Asynchronous_and_Synchronous_layout)
   4. [优化](#Optimizations)
   5. [布局处理](#The_layout_process)
   6. [宽度计算](#Width_calculation)
   7. [换行](#Line_Breaking)
6. 绘制
   1. [全局绘制和增量绘制](#Global_and_Incremental)
   2. [绘制顺序](#The_painting_order)
   3. [Firefox 显示列表](#Firefox_display_list)
   4. [WebKit 矩形存储](#Webkit_rectangle_storage)
7. [动态变化](#Dynamic_changes)
8. 呈现引擎的线程
   1. [事件循环](#Event_loop)
9. CSS2 可视化模型
   1. [画布](#The_canvas)
   2. [CSS 框模型](#CSS_Box_model)
   3. [定位方案](#Positioning_scheme)
   4. [框类型](#Box_types)
   5. 定位
      1. [相对定位](#Relative)
      2. [浮动定位](#Floats)
      3. [绝对定位和固定定位](#Absolute_and_fixed)
   6. [分层展示](#Layered_representation)
10. [资源](#Resources)
...
![img](images/layers.png)图：浏览器的主要组件。
...
![img](images/flow.png)图：呈现引擎的基本流程。
...
![img](images/webkitflow.png)图：WebKit 主流程

![img](images/image008.jpg)图：Mozilla 的 Gecko 呈现引擎主流程 ([3.6](#3_6))
...
![img](images/image009.png)图：数学表达式树节点
...
![img](images/image011.png)图：从源文档到解析树
...
![img](images/image013.png)图：编译流程
...
![img](images/image015.png)图：示例标记的 DOM 树
...
![img](images/image017.png)图：HTML 解析流程（摘自 HTML5 规范）
...
![img](images/image019.png)图：对示例输入进行标记化
...
![img](images/image022.gif)图：示例 HTML 的树构建
...
![img](images/image023.png)图：解析 CSS
...
![img](images/image025.png)图：呈现树及其对应的 DOM 树 ([3.1](#3_1))。初始容器 block 为“viewport”，而在 WebKit 中则为“RenderView”对象。
...
![img](images/image035.png)图：Firefox 样式上下文树 ([2.2](#2_2))
...
![img](images/reflow.png)图：增量布局 - 只有 dirty 呈现器及其子代进行布局 ([3.6](#3_6))。
...
[CSS2 规范定义了绘制流程的顺序](http://www.w3.org/TR/CSS21/zindex.html)。绘制的顺序其实就是元素进入的顺序。这些堆栈会从后往前绘制，因此这样的顺序会影响绘制。块呈现器的堆栈顺序如下：
...
在发生变化时，浏览器会尽可能做出最小的响应。因此，元素的颜色改变后，只会对该元素进行重绘。元素的位置改变后，只会对该元素及其子元素（可能还有同级元素）进行布局和重绘。添加 DOM 节点后，会对该节点进行布局和重绘。一些重大变化（例如增大“html”元素的字体）会导致缓存无效，使得整个呈现树都会进行重新布局和绘制。呈现引擎采用了单线程。几乎所有操作（除了网络操作）都是在单线程中进行的。在 Firefox 和 Safari 中，该线程就是浏览器的主线程。而在 Chrome 浏览器中，该线程是标签进程的主线程。网络操作可由多个并行线程执行。并行连接数是有限的（通常为 2 至 6 个，以 Firefox 3 为例是 6 个）。浏览器的主线程是事件循环。它是一个无限循环，永远处于接受处理状态，并等待事件（如布局和绘制事件）发生，并进行处理。这是 Firefox 中关于主事件循环的代码：
...
![img](images/image046.jpg)图：CSS2 框模型
...
![img](images/image065.png)图：相对定位
...
![img](images/image067.png)图：浮动

#### 绝对定位和固定定位

这种布局是准确定义的，与普通流无关。元素不参与普通流。尺寸是相对于容器而言的。在固定定位中，容器就是可视区域。

![img](images/image069.png)图：固定定位
...
![img](images/image071.png)图：固定定位
